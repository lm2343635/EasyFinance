...
//缩略图词缀
var THUMBNAIL_SUFFIX="thumbnail.jpg";
			...
			'<div class="modal-content" id="show-photos">'+
		        ...
		        '<div class="modal-body">'+
					...
					'<form id="fileupload" action="//jquery-file-upload.appspot.com/" method="POST" enctype="multipart/form-data">'+
						'<div class="row fileupload-buttonbar" id="upload-photos">'+
						    '<div class="col-lg-7">'+
		        ...
		        '<div class="modal-footer">'+
			        ...
			        '<button type="button" class="btn btn-success pull-left" id="upload-photos-btn">Upload Photos</button>'+
		        '</div>'+
			'</div>'+
		'</div>'+
	'</div>';
//全图浏览模式
var photosFullModal=
	'<div class="modal fade" id="photos-full-modal" tabindex="-1">'+
		'<div class="modal-dialog modal-lg">'+
			'<div class="modal-content" id="show-photos">'+
 ...
 * @param horizontalProportion 水平占比
 */
function showPhotos(srcs,horizontalProportion,thumbnail)
{
	//将当前路径赋值给全局变量
	Srcs=srcs;
	if(horizontalProportion==null)
	...
	for(var i in srcs)
	{
		tr+='<td class="thumbnail-td"><img src="'+getName(srcs[i])+"."+THUMBNAIL_SUFFIX+'" class="img-rounded show-photos-item" onclick="showFull('+i+')"></td>';
	...
	//绑定上传照片按钮点击事件
	$("#upload-photos-btn").click(function() {
		$("#show-photos-table").hide("normal");
		$("#upload-photos").show();
...
{
	$("#show-photos-modal").modal("hide");
	$("body").append(photosFullModal);
	$("#photos-full-modal").modal("show");
	
	$("#photos-full-modal").on("shown.bs.modal",function(e){
	...
	$("#photos-full-modal").on("hidden.bs.modal",function(e){
		$(this).remove();
	});
}
...
function loadPhotoController(selected,magnifying)
{
	...
	{
		var td='<img src="'+getName(Srcs[i])+"."+THUMBNAIL_SUFFIX+'" class="img-rounded ';
		if(i==selected)
